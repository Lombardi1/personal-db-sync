...
export function ModalAnagraficaForm({
  ...
  React.useEffect(() => {
    if (isOpen) {
      reset(normalizedInitialData as FormData);
      if (type === 'fornitore' && 'tipo_fornitore' in normalizedInitialData) {
        setValue('tipo_fornitore' as any, normalizedInitialData.tipo_fornitore);
      } else if (type === 'fornitore') {
        setValue('tipo_fornitore' as any, '');
      }
    }
  }, [isOpen, normalizedInitialData, reset, setValue, type]);
