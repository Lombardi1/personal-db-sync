...
import { Search, Plus, Home, Pencil, Trash2, Save, X, ChevronDown, ChevronUp } from 'lucide-react';
...
function SearchCombobox({
  ...
  renderValue?: (id: string) => string;
}) {
  const [open, setOpen] = useState(false);
  const selected = items.find((i) => i.id === value);
  const displayValue = renderValue ? renderValue(value) : (selected?.label ?? '');
  ...
  return (
    <Popover open={open} onOpenChange={setOpen}>
      <PopoverTrigger asChild>
        <button
          type="button"
          className="flex h-8 w-full items-center justify-between rounded-md border border-input bg-background px-3 py-1 text-sm shadow-sm hover:bg-accent"
        >
...
export default function DBArticoliProduzione() {
  ...
  const articoliFiltrati = articoli.filter((a) => {
    ...
    return (
      ...
      (a as any).codice_cliente?.toLowerCase().includes(s)
    );
  });
  ...
  const openEdit = (a: Articolo) => {
    ...
    setForm({
      ...
      iso_22000: r.iso_22000 ?? false,
      fsc: r.fsc ?? false,
      tipologia: r.tipologia || '',
      grammatura: r.grammatura || '',
      cartone_codice_magazzino: r.cartone_codice_magazzino || '',
      has_c: r.has_c ?? true,
      has_m: r.has_m ?? true,
      has_y: r.has_y ?? true,
      has_k: r.has_k ?? true,
      ...
      pan_nr_5: r.pan_nr_5 || '',
      polimero: r.polimero || '',
      polimero_codice_magazzino: r.polimero_codice_magazzino || '',
      polimero_mode: r.polimero_codice_magazzino ? 'magazzino' : 'nuovo',
      finitura: r.finitura || '',
      uv: r.uv || '',
      polimero_uv: r.polimero_uv || '',
      linear: r.linear || '',
      ha_terzista: r.ha_terzista ?? false,
      terzista: r.terzista || '',
      lavorazione: r.lavorazione || '',
      terzista_2: r.terzista_2 || '',
      lavorazione_2: r.lavorazione_2 || '',
      terzista_3: r.terzista_3 || '',
      lavorazione_3: r.lavorazione_3 || '',
      terzista_4: r.terzista_4 || '',
      lavorazione_4: r.lavorazione_4 || '',
      ha_fustella: r.ha_fustella ?? false,
      fustella_nr: r.fustella_nr || '',
      pulitore_codice: r.pulitore_codice || '',
      ha_finestratura: r.ha_finestratura ?? false,
      h_finestratura: r.h_finestratura || '',
      tipologia_finestratura: r.tipologia_finestratura || '',
      ha_incollatura: r.ha_incollatura ?? false,
      macchina_incollatura: r.macchina_incollatura || '',
      tipologia_incollatura: r.tipologia_incollatura || '',
      terzista_incollatura: r.terzista_incollatura ?? false,
      scatolone: r.scatolone || '',
      quantita_per_scatolone: r.quantita_per_scatolone || '',
      peso: r.peso || '',
      bancale: r.bancale || '',
    });
    setDialogOpen(true);
  ...
  const handleSave = async () => {
    ...
    try {
      const payload: Record<string, any> = {
        ...
        iso_22000: form.iso_22000,
        fsc: form.fsc,
        tipologia: form.tipologia,
        grammatura: form.grammatura,
        cartone_codice_magazzino: form.cartone_codice_magazzino || null,
        has_c: form.has_c,
        has_m: form.has_m,
        has_y: form.has_y,
        has_k: form.has_k,
        pan_nr: form.pan_nr,
        pan_nr_2: form.pan_nr_2,
        pan_nr_3: form.pan_nr_3,
        pan_nr_4: form.pan_nr_4,
        pan_nr_5: form.pan_nr_5,
        ...
        finitura: form.finitura,
        uv: form.uv,
        polimero_uv: form.polimero_uv,
        linear: form.linear,
        ha_terzista: form.ha_terzista,
        terzista: form.ha_terzista ? form.terzista : null,
        lavorazione: form.ha_terzista ? form.lavorazione : null,
        terzista_2: form.ha_terzista ? form.terzista_2 : null,
        lavorazione_2: form.ha_terzista ? form.lavorazione_2 : null,
        terzista_3: form.ha_terzista ? form.terzista_3 : null,
        lavorazione_3: form.ha_terzista ? form.lavorazione_3 : null,
        terzista_4: form.ha_terzista ? form.terzista_4 : null,
        lavorazione_4: form.ha_terzista ? form.lavorazione_4 : null,
        ha_fustella: form.ha_fustella,
        fustella_nr: form.ha_fustella ? form.fustella_nr : null,
        pulitore_codice: form.ha_fustella ? form.pulitore_codice : null,
        ha_finestratura: form.ha_finestratura,
        h_finestratura: form.ha_finestratura ? form.h_finestratura : null,
        tipologia_finestratura: form.ha_finestratura ? form.tipologia_finestratura : null,
        ...
        scatolone: form.scatolone,
        quantita_per_scatolone: form.quantita_per_scatolone,
        peso: form.peso,
        bancale: form.bancale,
      };
  ...
  const giacenzeItems = giacenze.map((g) => ({
    id: g.codice,
    label: g.codice,
    sub: [g.tipologia, g.grammatura, g.formato, g.magazzino].filter(Boolean).join(' · '),
  }));

  const polimeriItems = polimeri.map((p) => ({
    id: p.codice,
    label: p.codice,
    sub: [p.cliente, p.lavoro].filter(Boolean).join(' · '),
  }));

  const fustelleItems = fustelle.map((f) => ({
    id: f.codice,
    label: f.codice,
    sub: [f.cliente, f.lavoro, f.resa].filter(Boolean).join(' · '),
  }));
              ...
              <Table>
                ...
                <TableBody>
                  {loading ? (
                    ...
                    <TableRow><TableCell colSpan={7} className="text-center py-8">Nessun articolo trovato</TableCell></TableRow>
                  ) : (
        ...
        <DialogContent className="max-w-3xl max-h-[90vh] overflow-y-auto">
          ...
          <div className="space-y-8 py-4">
            ...
            <div>
              ...
              <div className="flex gap-6">
                <label className="flex items-center gap-2 cursor-pointer select-none">
                  <Checkbox checked={form.iso_22000} onCheckedChange={(v) => set('iso_22000', !!v)} />
                  <span className="text-sm font-medium">ISO 22000</span>
                </label>
                <label className="flex items-center gap-2 cursor-pointer select-none">
                  <Checkbox checked={form.fsc} onCheckedChange={(v) => set('fsc', !!v)} />
                  <span className="text-sm font-medium">FSC</span>
                </label>
              </div>
            </div>
            ...
            <div>
              ...
              {form.polimero_mode === 'nuovo' ? (<Input value={form.polimero} onChange={(e) => set('polimero', e.target.value)} placeholder="Codice / descrizione polimero" className="h-8 text-sm mb-4" />)
              ...
              {form.ha_terzista && (
                <div className="space-y-2">
                  {([
                    ...
                    ['terzista_3', 'lavorazione_3'],
                    ['terzista_4', 'lavorazione_4'],
                  ] as [keyof FormData, keyof FormData][]).map(([tk, lk], i) => (
