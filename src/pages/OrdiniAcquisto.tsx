...
export default function OrdiniAcquisto() {
  ...
  const {
    ordiniAcquisto, loading, addOrdineAcquisto, updateOrdineAcquisto,
    cancelOrdineAcquisto, deleteOrdineAcquistoPermanently, loadOrdiniAcquisto,
    updateOrdineAcquistoStatus, updateArticleStatusInOrder
  } = useOrdiniAcquisto();
  ...
  const [filtri, setFiltri] = useState({ numero_ordine: '', fornitore_nome: '', stato: '', data_ordine_filter: '', fornitore_tipo: '' });
  ...
  useEffect(() => { handleFilter(filtri); }, [ordiniAcquisto, filtri]);
      ...
      if (value) {
        if (key === 'data_ordine_filter') {
          filtered = filtered.filter(o => {
            ...
            return orderDate.getFullYear().toString().includes(filterValue) || o.data_ordine.includes(filterValue);
          });
        } else if (key === 'fornitore_tipo') {
          filtered = filtered.filter(o => String(o.fornitore_tipo).toLowerCase().includes(value.toLowerCase()));
        } else if (key === 'stato') {
          filtered = filtered.filter(o => o.articoli.some(articolo => articolo.stato === value));
        } else {
          filtered = filtered.filter(o => {
  ...
  const resetFiltri = () => {
    const emptyFiltri = { numero_ordine: '', fornitore_nome: '', stato: '', data_ordine_filter: '', fornitore_tipo: '' };
  ...
  const confirmDuplicateAndEdit = async () => {
    ...
    const ordersThisYear = ordiniAcquisto.filter(o => o.numero_ordine && o.numero_ordine.endsWith(`/${currentYearShort}`));
    let maxSeqNum = 0;
    ordersThisYear.forEach(o => {
      const parts = o.numero_ordine?.split('/');
      if (parts && parts.length === 2) {
        const seqNum = parseInt(parts[0]);
        if (!isNaN(seqNum) && seqNum > maxSeqNum) maxSeqNum = seqNum;
      }
    });
    const newNumeroOrdine = `${maxSeqNum + 1}/${currentYearShort}`;
    ...
    duplicatedOrder.articoli = duplicatedOrder.articoli.map(art => {
      ...
      if (isCartoneFornitore) newArticle.codice_ctn = generateNextCartoneCode();
      newArticle.stato = 'in_attesa';
      return newArticle;
    });
    duplicatedOrder.importo_totale = duplicatedOrder.articoli.reduce((sum, item) => {
      if (item.stato !== 'annullato') return sum + ((item.quantita || 0) * (item.prezzo_unitario || 0));
      return sum;
    }, 0);
  ...
  const handleCloseModal = () => { setIsModalOpen(false); setEditingOrdine(null); };
  ...
  if (loading || anagraficheLoading || aziendaInfoLoading) {
    return (
  ...
  return (
    <div className="min-h-screen bg-[hsl(210,40%,96%)]">
      <Header title="Gestione Ordini d'Acquisto" activeTab="ordini-acquisto" showUsersButton={true} />
      <div className="max-w-[1400px] mx-auto p-3 sm:p-5 md:px-8">
        ...
        <div className="flex flex-col sm:flex-row items-center justify-between mb-4 sm:mb-6 gap-3 sm:gap-0">
          ...
          {!isVisualizzatore && (
            <Button onClick={() => { setEditingOrdine(null); setIsModalOpen(true); }} size="sm"
        ...
        <Filters filtri={filtri} onFilter={handleFilter} onReset={resetFiltri} matchCount={ordiniFiltered.length} sezione="ordini-acquisto" />
        ...
        {ordiniAcquisto.length === 0 ? (
          <div className="flex flex-col items-center justify-center min-h-[calc(100vh-400px)] bg-white rounded-lg shadow-sm p-6 border border-[hsl(var(--border))]">
            ...
            <p className="text-base sm:text-lg text-[hsl(var(--muted-foreground))] text-center max-w-prose">
              Nessun ordine d'acquisto registrato.{!isVisualizzatore && ' Clicca "Nuovo Ordine" per aggiungerne uno.'}
            </p>
          </div>
        ) : (
          <TabellaOrdiniAcquisto
            ordini={ordiniFiltered}
            onEdit={isVisualizzatore ? () => {} : (ordine) => { setEditingOrdine(ordine); setIsModalOpen(true); }}
            onCancel={isVisualizzatore ? () => {} : cancelOrdineAcquisto}
            onPermanentDelete={isVisualizzatore ? () => {} : deleteOrdineAcquistoPermanently}
            onDuplicateAndEdit={isVisualizzatore ? () => {} : handleDuplicateClick}
            fornitori={fornitori}
            clienti={clienti}
            aziendaInfo={aziendaInfo}
            updateOrdineAcquistoStatus={isVisualizzatore ? async () => {} : updateOrdineAcquistoStatus}
            updateArticleStatusInOrder={isVisualizzatore ? async () => {} : updateArticleStatusInOrder}
            isVisualizzatore={isVisualizzatore}
          />
        )}
      </div>

      {!isVisualizzatore && isModalOpen && (
        ...
        <AlertDialog open={isDuplicateConfirmOpen} onOpenChange={setIsDuplicateConfirmOpen}>
          <AlertDialogContent>
            ...
            <AlertDialogFooter>
              ...
              <AlertDialogAction onClick={confirmDuplicateAndEdit}>Duplica Ordine</AlertDialogAction>
            </AlertDialogFooter>
          </AlertDialogContent>
        </AlertDialog>
      )}
