...
-- Create the app_users table if it doesn't exist
    ...
    CREATE TABLE IF NOT EXISTS public.user_roles (
        ...
        role text NOT NULL CHECK (role IN ('produzione', 'amministratore')),
        created_at timestamp with time zone DEFAULT now() NOT NULL
    );

    -- Create or replace the auth.role() function
    CREATE OR REPLACE FUNCTION auth.role()
     RETURNS text
     LANGUAGE plpgsql
     SECURITY DEFINER
    AS $function$
        SELECT r.role FROM public.user_roles r JOIN public.app_users u ON r.user_id = u.id WHERE u.id = auth.uid();
    $function$;
